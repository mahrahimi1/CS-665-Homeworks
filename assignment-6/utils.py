...
def evaluate_model_regression(model, dataset):
    print("\nEvaluating...")
    training_MSE = calc_avg_loss(model , dataset.training_set)
    validation_MSE = calc_avg_loss(model , dataset.validation_set)
    test_MSE = calc_avg_loss(model , dataset.testing_set)
    print("Evaluation complete:")
    print("  Average Training MSE:    %.2f" % (training_MSE))
    print("  Average Validation MSE:    %.2f" % (validation_MSE))
    print("  Average Test MSE:    %.2f" % (test_MSE))
    
def calc_avg_loss(model , set):
    loss = 0
    for i in range(len(set)):
        sample = set[i]
        pred = dot(sample.features, model.weights)
        l_sample = model.loss_f.loss(pred , sample)
        loss = loss + l_sample
    loss = (loss / len(set))
    return loss
